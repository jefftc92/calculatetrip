...
function buildKeyDifferences(a, b) {
  ...
  RATING_ROWS.filter(r => r.key !== 'overall').forEach(({ key, label }) => {
    ...
    if (sA !== null && sB !== null) {
      if (sA - sB >= 0.3) aStronger.push(label.toLowerCase())
      else if (sB - sA >= 0.3) bStronger.push(label.toLowerCase())
    }
  })

  const lines = []
  lines.push(
    `${winner.name} (${winner.ratings.overall}/10) edges ahead of ${loser.name} (${loser.ratings.overall}/10) overall, but both rank among the top all-inclusive resorts in the Caribbean region.`
  )
  if (aStronger.length > 0) {
    lines.push(`${a.name} leads on ${aStronger.slice(0, 3).join(', ')}.`)
  }
  if (bStronger.length > 0) {
    lines.push(`${b.name} pulls ahead on ${bStronger.slice(0, 3).join(', ')}.`)
  }
  if (a.type !== b.type) {
...
function buildResortSummary(resort, opponent) {
  const bullets = []
...
function SectionToggle({ title, children, defaultOpen = false }) {
  return (
  ...
  return (
    <>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 -mt-8 pb-16">
            ...
            {[{ resort: a, bullets: aSummary }, { resort: b, bullets: bSummary }].map(({ resort, bullets }) => (
              <div key={resort.slug} className="bg-ocean-50/60 rounded-xl p-4">
                ...
                <ul className="space-y-1.5">
                  {bullets.map((b) => (
                    <li key={b} className="flex items-start gap-2 font-sans text-sm text-ocean-700">
        ...
        {/* ── Ratings comparison (collapsible, open) ── */}
            ...
            return (
              ...
              >
                ...
                <div className={`py-3.5 flex items-center justify-center ${aRowWins ? 'bg-emerald-50' : ''}`}>
                  {sA !== null ? (
                    <div className="text-center">
                      <span className={`font-sans text-base font-bold tabular-nums ${scoreColor(sA)} ${aRowWins ? 'text-lg' : ''}`}>{sA}</span>
                ...
                <div className={`py-3.5 flex items-center justify-center ${bRowWins ? 'bg-emerald-50' : ''}`}>
                  {sB !== null ? (
                    <div className="text-center">
                      <span className={`font-sans text-base font-bold tabular-nums ${scoreColor(sB)} ${bRowWins ? 'text-lg' : ''}`}>{sB}</span>
