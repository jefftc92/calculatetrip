...
export async function generateMetadata({ params }) {
  ...
  const description = `Head-to-head comparison: ${a.name} (${a.ratings.overall}/10) vs ${b.name} (${b.ratings.overall}/10). Which all-inclusive wins on food, beach, pool, value, and service?`
  return {
    title, description,
    ...
    <>
      ...
      <div className="bg-ocean-950 pt-8 pb-14">
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          ...
          <p className="font-sans text-ocean-400 mt-3 text-sm">All-inclusive resort comparison · 2025</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-10">
          {[a, b].map((r) => (
            ...
            >
              ...
              <Link href={`/resorts/${r.slug}/`} className="font-serif text-base font-bold text-ocean-950 hover:text-ocean-700 transition-colors block leading-snug mb-1">
                {r.name}
              </Link>
              <p className="font-sans text-xs text-ocean-400 mb-3">{r.country} · {r.type === 'adults-only' ? 'Adults Only' : 'Family'}</p>
        ...
        <div className="bg-ocean-50 border border-ocean-100 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {resorts
              ...
              .map((link) => (
                <Link key={link.href} href={link.href} className="font-sans text-sm text-ocean-700 hover:text-ocean-950 hover:underline transition-colors">
                  {link.label} →
                </Link>
              ))}
          </div>
