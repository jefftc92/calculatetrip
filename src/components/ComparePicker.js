...
function SlotCard({ resort, onClick, onClear, disabled, label }) {
  return (
...
export default function ComparePicker({ resorts }) {
  ...
  const [modalOpen, setModalOpen] = useState(null) // 'a' | 'b' | null
  ...
  function pick(slug) {
    if (modalOpen === 'a') {
      setSlotA(slug)
      // Auto-advance to slot B if it's empty
      if (!slotB) {
        setModalOpen('b')
      } else {
        setModalOpen(null)
  ...
  // Resorts available for each slot (exclude the other slot's selection)
  const available = modalOpen === 'a'
  ...
  return (
    <>
      ...
      {/* Compare button — appears once both slots are filled */}
      {slotA && slotB && (
        <button
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-ocean-100">
              <div>
                ...
                <p className="font-sans text-xs text-ocean-400 mt-0.5">
                  {modalOpen === 'a' ? 'Select your first resort' : `Comparing with ${resortA?.name.split(' ').slice(0, 2).join(' ')}`}
                </p>
              </div>
              <button
                onClick={() => setModalOpen(null)}
